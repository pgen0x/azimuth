...
type ModeParams struct {
	...
	MinQuoteOrganic float64 // quote-token organic floor (ported from Reference-bot)
...
// Bin-step band (80–125) ported from Reference-bot config; tune per strategy.
//
...
// Ported from Reference-bot; inputs are normalized to a 30m reference window.
const (
...
func Screen(p Pool, mp ModeParams) (*Candidate, string) {
	...
	// Authoritative API risk flags (ported from Reference-bot) — cheaper than parsing
	...
	// Bin-step band gate (ported from Reference-bot). 0 endpoints disable each side.
...
func degenScore(p Pool, tfMinutes float64) float64 {
	...
	// window volume/fee (mirrors Reference-bot). Without this, a missing ratio
