...
*   Rug blacklist: a realized close <= -30% PnL, OR any close whose reason contains "rug" (e.g. the velocity gate above, regardless of realized PnL — a fast reaction can book a near-zero loss on a token that just cratered) — blocks the mint for `RUG_BLACKLIST_TTL_DAYS` (default 7d) via one TTL'd key per mint, `sol:dlmm:blocklist:mint:<MINT>`, and the deployer permanently via the `sol:dlmm:blocklist:dev` set. Both checked by the pipeline before every deploy. The mint side was a permanent set until 2026-08-07 — discovery recycles a small ticker pool, so a monotonic blocklist eventually starves the mode of candidates. A re-blacklist refreshes the window, so a repeat rug keeps its ban.
